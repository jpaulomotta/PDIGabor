...
public class ImpressaoDigital {
	...
	public static int[][] getCN(int skeleton[][])//after thinning
	{
		...
		for(i=trimT;i<skeleton.length-1;i++)
			for(j=trimL;j<skeleton[0].length-1;j++)
			{
				...
				if(CN[i-trimT][j-trimL] > 0) System.out.println(CN[i-trimT][j-trimL]);
			}
		return CN;
	}
	...
	public static BufferedImage matrixToBuffer(int[][] imgBin) {
		...
		for(i=1;i<imgBin.length-1;i++) {
			for(j=1;j<imgBin[i].length-1;j++) {
				if(imgBin[i][j]==0)
					debugImg.setRGB(i, j, new Color(255, 255, 255).getRGB());
				else
					debugImg.setRGB(i, j, new Color(0, 0, 0).getRGB());
	...
	public static BufferedImage reconhecerTracos(BufferedImage imagem)
	{
		/*node[] graph1=
				genGraph( 
						getCN(
		...
		try {
			return //getCN(
					matrixToBuffer(getFastParallelAlgoSkeleton(
						binarizing(
								imagem)));
		} catch(Exception ex) {
